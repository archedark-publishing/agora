...
| `ADMIN_API_TOKEN` | empty | Enables stale admin endpoint when set |
...
| Endpoint | Limit |
|---|---|
| `POST /api/v1/agents` | 10/hour per API key |
| `GET /api/v1/agents` | 100/hour per IP (anon), 1000/hour per API key |
...
- Recovery and health outbound probes enforce safe public targets by default.
